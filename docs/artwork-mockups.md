# Mockups: putting a real Tabbied artwork onto a real-looking object
...
`docs/image-pipeline.md`, which covers the ordinary showcase photography.
...
with the artwork. It cannot execute CSS, so it is guessing from a description.
...
**Rendering the artwork and passing the pixels does work.** The artwork is
...
```
  packages/tabbied/artworks/<design>.json
       │  scripts/render-artwork.mjs   (headless Chromium, real css-doodle)
       ▼
  generated-images/refs/<id>-ref.png          ← the artwork, exact pixels. Free.
...
a van or a tote; it is not fine if you need the exact artwork.

| Tier | Technique | Pattern fidelity | Use for |
|---|---|---|---|
| 0 | Render the live `<TabbiedArtwork>` in the page | exact, and animated | anything on the site itself |
...
```bash
node scripts/render-artwork.mjs --design bauhaus --out refs/poster.png \
...
css-doodle in Playwright, mounts the artwork and screenshots the element at
...
An importmap in `scripts/render-artwork.html` maps the bare `css-doodle`
...
> A product photograph of *[subject]*. **The artwork in the provided image is
...
existing showcase imagery is unaffected.
...
1. Add an entry to `data/mockup-prompts.json`: an `id`, a `subject` (the scene,
   no style or pattern words), a `size`, and an `artwork` block naming the
...
- A tier-2 mockup is a **faithful impression, not a reproduction**. Do not
  ...
  site renders live artworks anyway) and the difference reads as photography
