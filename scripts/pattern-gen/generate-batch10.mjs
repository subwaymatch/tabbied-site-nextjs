// Syncs packages/tabbied/patterns/ with the batch-10 definitions: writes one
// JSON per definition, deletes any batch-10 pattern (and its gallery thumbnail
...
import { batch10 } from './pattern-defs-10.mjs';
...
// Guard: batch-10 slugs must be unique within the batch and must not clobber any
...
for (const def of defs) {
  ...
  if (!ownedByBatch10(orderOf(def.slug))) {
    throw new Error(`batch-10 slug ${def.slug} collides with an existing pattern`);
  }
}
...
const dropped = [...existing].filter(
  (slug) => !batchSlugs.has(slug) && ownedByBatch10(orderOf(slug))
);
