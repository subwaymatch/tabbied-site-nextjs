// Syncs packages/tabbied/patterns/ with the batch-12 definitions: writes one
// JSON per definition, deletes any batch-12 pattern (and its gallery thumbnail
...
for (const def of defs) {
  ...
  if (!ownedByBatch12(orderOf(def.slug))) {
    throw new Error(`batch-12 slug ${def.slug} collides with an existing pattern`);
  }
}
