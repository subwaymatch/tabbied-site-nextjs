// Syncs packages/tabbied/patterns/ with the batch-7 definitions: writes one
// JSON per definition, deletes any batch-7 pattern (and its gallery thumbnail
...
for (const def of defs) {
  ...
  if (!ownedByBatch7(orderOf(def.slug))) {
    throw new Error(`batch-7 slug ${def.slug} collides with an existing pattern`);
  }
}
