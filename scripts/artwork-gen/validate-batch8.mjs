// Renders every batch-8 artwork in headless Chromium at its gallery-thumbnail
...
import { batch8 } from './artwork-defs-8.mjs';
...
const only = process.env.ONLY
  ...
  : null;
const defs = only ? batch8.filter((d) => only.has(d.slug)) : batch8;
if (!defs.length) throw new Error('ONLY matched no batch-8 slugs');
...
for (const chunk of chunks) {
  ...
  await page.screenshot({ path: `/tmp/sheet-b8-${shot}-seed1.png`, fullPage: true });
  ...
  await page.screenshot({ path: `/tmp/sheet-b8-${shot}-seed2.png`, fullPage: true });
  await page.close();
  ...
  await clearPage.screenshot({ path: `/tmp/sheet-b8-${shot}-clear.png`, fullPage: true });
...
console.log(
  `all ${defs.length} batch-8 artworks render, re-draw on reseed and render identically on a transparent background ✓`
);
