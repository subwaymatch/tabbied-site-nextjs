// Syncs packages/tabbied/artworks/ with the batch-10 definitions: writes one
// JSON per definition, deletes any batch-10 artwork (and its gallery thumbnail
...
for (const def of defs) {
  ...
  if (!ownedByBatch10(orderOf(def.slug))) {
    throw new Error(`batch-10 slug ${def.slug} collides with an existing artwork`);
  }
}
