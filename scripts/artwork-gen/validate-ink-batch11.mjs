...
//
// Usage: node scripts/artwork-gen/validate-ink-batch11.mjs
...
const buildSource = (artwork, grid) => {
...
for (let i = 0; i < slugs.length; i += PER_PAGE) {
  ...
  const blocks = chunk.map((slug) => {
    ...
    return { slug, background: artwork.palette[0], ...buildSource(artwork, gridFor(slug)) };
  });
...
console.log(`\nall ${slugs.length} artworks put readable ink on the sheet ✓`);
