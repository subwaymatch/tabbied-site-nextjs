// Syncs packages/tabbied/artworks/ with the batch-8 definitions: writes one
// JSON per definition, deletes any batch-8 artwork (and its gallery thumbnail
...
for (const def of defs) {
  ...
  if (!ownedByBatch8(orderOf(def.slug))) {
    throw new Error(`batch-8 slug ${def.slug} collides with an existing artwork`);
  }
}
