// Renders every batch-11 artwork in headless Chromium at its gallery-thumbnail
...
//
// This is the *rendering* half of batch 11's gate; validate-svg-batch11.mjs is
// the export half. Geometry is allowed to move on reseed — @rand() and
// @pick() place the shapes — but background independence is not optional:
// re-rendering with the background slot set to a zero-alpha color must produce
// byte-identical cells. A design that knocked its holes out with var(--color0)
// would quietly fill them in here. (Same seed → same rolls, so the two passes
// are directly comparable even though the design is random.)
//
// Contact sheets land in /tmp/sheet-b11-*.png — the transparent pass is shot
...
const only = process.env.ONLY
  ...
  : null;
const defs = only ? batch11.filter((d) => only.has(d.slug)) : batch11;
if (!defs.length) throw new Error('ONLY matched no batch-11 slugs');
...
for (const chunk of chunks) {
  ...
  await page.screenshot({ path: `/tmp/sheet-b11-${shot}-seed1.png`, fullPage: true });
  ...
  await page.screenshot({ path: `/tmp/sheet-b11-${shot}-seed2.png`, fullPage: true });
  await page.close();
  ...
  await clearPage.screenshot({ path: `/tmp/sheet-b11-${shot}-clear.png`, fullPage: true });
...
console.log(
  `all ${defs.length} batch-11 artworks render, re-draw on reseed and render identically on a transparent background ✓`
);
