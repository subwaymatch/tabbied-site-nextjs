// Syncs packages/tabbied/artworks/ with the batch-9 definitions: writes one
// JSON per definition, deletes any batch-9 artwork (and its gallery thumbnail
...
// Guard: batch-9 slugs must be unique within the batch and must not clobber any
...
for (const def of defs) {
  ...
  if (!ownedByBatch9(orderOf(def.slug))) {
    throw new Error(`batch-9 slug ${def.slug} collides with an existing artwork`);
  }
}
...
const dropped = [...existing].filter(
  (slug) => !batchSlugs.has(slug) && ownedByBatch9(orderOf(slug))
);
