// Renders every batch-10 artwork in headless Chromium at its gallery-thumbnail
...
import { batch10 } from './artwork-defs-10.mjs';
...
const only = process.env.ONLY
  ...
  : null;
const defs = only ? batch10.filter((d) => only.has(d.slug)) : batch10;
if (!defs.length) throw new Error('ONLY matched no batch-10 slugs');
...
for (const chunk of chunks) {
  ...
  await page.screenshot({ path: `/tmp/sheet-b10-${shot}-seed1.png`, fullPage: true });
  ...
  await clearPage.screenshot({ path: `/tmp/sheet-b10-${shot}-clear.png`, fullPage: true });
...
console.log(
  `all ${defs.length} batch-10 artworks render, re-draw on reseed and render identically on a transparent background ✓`
);
