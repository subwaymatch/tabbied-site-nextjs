...
// `svgExportNote`, no converter warning.
//
...
//      house 1% budget, using the same anti-aliasing-tolerant diff as
...
const ARTIFACTS =
  process.env.SVG_ARTIFACTS || path.join(os.tmpdir(), 'tabbied-svg-b11');
mkdirSync(ARTIFACTS, { recursive: true });

// The tolerance matches e2e/svg-export.spec.ts, but the budget is deliberately
// tighter than the shipped 1%: batch 11 gets no per-artwork headroom, and a
// design that needs more than a rounding edge's worth of slack has an
...
function buildPage(blocks, seed) {
...
for (const slug of slugs) {
  if (!known.has(slug)) throw new Error(`no artwork JSON for ${slug} — run the generator first`);
}
...
const gridFor = (slug) => {
  const def = batch11.find((d) => d.slug === slug);
  return def?.thumb.grid ?? '4x4';
};
...
for (const seed of SEEDS) {
  for (let i = 0; i < slugs.length; i += PER_PAGE) {
    ...
    pageErrors.length = 0;
    await page.setContent(buildPage(blocks, seed), { waitUntil: 'load' });
    ...
    for (const slug of chunk) {
      ...
      if (r.error || r.badFraction > MAX_BAD_FRACTION) {
        ...
        for (const [name, uri] of [['mine.png', r.minePng], ['ref.png', r.refPng]]) {
          if (uri) writeFileSync(path.join(dir, name), Buffer.from(uri.split(',')[1], 'base64'));
        }
      }
    }
...
console.log(
  `\nall ${slugs.length} artworks export as native SVG with no warnings and match their live render (${checked} checks) ✓`
);
