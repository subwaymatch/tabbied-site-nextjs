...
- **Date:** 2026-07-29
- **Repo:** `subwaymatch/tabbied` (branch: `claude/pattern-svg-export-gdohv5`, based on `95127ff`)
- **Audience:** a coding agent (or developer) implementing the feature. This document contains verified facts about the current codebase and `css-doodle` 0.51.0, the full pattern-compatibility audit, the chosen design, a phased implementation plan, and a verification harness that can prove correctness per pattern.
...
>    must be unwound in two steps or the patterns' 400ms transitions restart
...
>    three nested-@doodle patterns (`fractal`, `matryoshka`, `subdivide`)
...
>    patterns** with zero failures. (CI's Chromium build measures slightly
...
- **165 of 168 patterns are convertible.** The 3 that are not (`coil`, `spectrum`, `pinwheel` — they paint smooth conic color sweeps, which SVG cannot represent) get a new `"svgExport": false` flag in their JSON definitions, and the site disables the menu item with an explanatory tooltip.
- The exporter lives in the **`tabbied` package core** (`packages/tabbied/src/core/`), surfaced as `controller.exportSvg()` and `TabbiedPatternHandle.exportSvg()`, so both the site and package consumers get it.
...
- Correctness is provable: with a fixed seed the DOM snapshot is deterministic, so a Playwright harness can rasterize the exported SVG and pixel-diff it against css-doodle's own PNG export of the identical DOM. Every supported pattern must pass this parity test.
...
- Pattern presets are JSON files in `packages/tabbied/patterns/*.json` (168 files). `packages/tabbied/scripts/codegen.mjs` embeds each JSON **verbatim** as a named export in `src/patterns.generated.ts` (so a new JSON field flows through automatically; only the `PatternDefinition` type needs updating).
- `packages/tabbied/src/core/doodleSource.ts` builds the css-doodle source string (option substitution, palette → `--color0..N` custom properties). `createPattern.ts` mounts a `<css-doodle>` element and returns a `PatternController`.
- The React wrapper `packages/tabbied/src/react/TabbiedPattern.tsx` exposes an imperative handle: `redraw()`, `exportImage()` (PNG via css-doodle), and `element` (the raw `<css-doodle>`).
- The site's editor (`components/edit-pattern-page/EditPattern.tsx`) calls `exportImage({ scale: ceil(3000/max(w,h)), download: true })` from `exportPattern()` (~line 642). Export UI lives in two places: the desktop dropdown in `EditPatternHeader.tsx` (~line 164, Base UI `Menu`) and the mobile export panel in `EditPattern.tsx` (`renderExportPanel`, ~line 906).
...
#shadow-root (open)
  ...
  <b></b>            ← only when a @backdrop is defined (none of our patterns use it)
```
...
- Relevant nugget: the same `svg()` generator from `css-doodle/generator` powers the `@svg(...)` backgrounds inside 5 patterns (`charcoal`, `drypoint`, `linocut`, `reedpen`, `wireframe`) — those backgrounds are already real SVG data-URIs at runtime, so the exporter gets them nearly for free (§4.7).

---

## 2. Pattern compatibility audit (all 168 presets)

Audited by scanning every JSON's generated CSS and reading the actual gradient declarations of every flagged pattern. Screenshots of all 19 flagged patterns: `agent-outputs/native-svg-export-audit.png`.
...
| `pinwheel` | conic-gradient that *looks* hard-stop in source, but css-doodle re-rolls each `var()` occurrence independently (per-occurrence `@p()` rolls), so adjacent stops get different colors and the quarters blend smoothly — confirmed by the pattern's own code comment ("the quarters blend like a color wheel — intentional") and by rendering |
...
Everything else. Property usage across all 168 (whole-file scan): transforms 108, clip-path 80, pseudo-elements 67, border-radius 54, masks 28, repeating-linear-gradient 12, radial-gradient 12, linear-gradient 19, borders 5, z-index 7, `@svg()` backgrounds 5, `@shape` (compiles to clip-path polygons) 3. **Zero** patterns use text content, web fonts, shaders, `paint()`, `backdrop-filter`, or `outline` — none of those need handling.
...
export function supportsSvgExport(pattern: PatternDefinition): boolean; // === pattern.svgExport !== false
```

Wire-through (mirrors the existing `exportImage` plumbing exactly):

- `PatternController.exportSvg(options?: { download?: boolean; name?: string } & SvgExportOptions): Promise<SvgExportResult>` in `createPattern.ts` — waits for render/transition settle (§3.4), calls `doodleToSvg`, and when `download: true` builds a `Blob('image/svg+xml')` + anchor click named `<slug>.svg` (follow `get_png_name`'s timestamped naming convention if trivial, else `<slug>.svg`).
- `TabbiedPatternHandle.exportSvg(...)` in `TabbiedPattern.tsx`, rejecting before mount like `exportImage` does.
- Re-export everything from `core/index.ts`. Keep zero dependencies (the package currently has none — preserve that).

### 3.2 Gating unsupported patterns

- Add to `PatternDefinition` in `packages/tabbied/src/core/types.ts`:
  ```ts
...
- Site UI: in `EditPatternHeader.tsx`, add a "Download SVG" `Menu.Item` under "Download PNG", `disabled` when `!supportsSvgExport(pattern)` with a tooltip/`title` — copy: **"This design uses effects SVG can't represent."** Same treatment in the mobile export panel (`renderExportPanel` in `EditPattern.tsx`). Match existing row styling (`styles.exportRow`, `styles.menuItem`) and add a disabled style to the two module.css files. Toasts: "SVG downloaded" / "Could not export the SVG" via the existing `toaster`.
...
- Pseudo-elements are real paint layers here (67 patterns): read `getComputedStyle(el, '::before'|'::after')`; a pseudo exists iff `content` computes to something other than `none`. Its box derives from the computed `inset`/`left`/`top`/`width`/`height`/`position` against the parent box (all absolute-positioned in our patterns; `content` is always `''` — verified, no text anywhere).

### 3.4 Snapshot timing (transitions)

Many patterns declare `transition: ease 400ms`; a mid-transition computed read would capture intermediate values. Before walking, settle: collect `shadowRoot.querySelectorAll('*')`, gather `el.getAnimations()`, and `await Promise.allSettled(animations.map(a => a.finished))` with a ~1s timeout guard. (The editor's export runs from an idle UI, so this is a rare edge, but it's cheap insurance and keeps the API safe for package consumers who export right after `redraw()`.)
...
- First paint: host background — the pattern's `--color0` (read the host's computed `background-color`; fully transparent when the editor's transparent-background toggle is on — SVG preserves that naturally).
...
- Unknown feature encountered (e.g. a future pattern with an unhandled `background-image` kind): **fail loudly** — throw `SvgExportUnsupportedError` naming the property/value. Silent wrong output is the failure mode to avoid; the site catches and shows the error toast.

---

## 4. Feature → SVG mapping spec

Walk order per cell (this is CSS paint order for these patterns): element background layers → `::before` → child cells (depth grids) → `::after`. Cells paint in DOM order; 7 patterns use `z-index` — sort siblings by (z-index, DOM order) before emitting.
...
- `radial-gradient` (12): `<radialGradient>` with `cx cy r` from the computed position/size (the patterns use simple circle/ellipse forms; ellipse via `gradientTransform="scale"` trick or `rx/ry`-emulating transform). `repeating-radial-gradient`: `spreadMethod="repeat"`.
- Hard-stop **conic** gradients (Tier B only): not emitted as gradients at all — decomposed into sector paths (§4.6).
- Multiple `background-image` layers (1 pattern + any background + gradient combos): CSS lists layers **top-first**; emit in reverse so the first layer paints last. Respect `background-size`/`background-position`/`background-repeat` for gradient layers (computed, px-resolved): a sized+repeated gradient layer maps to a `<pattern>` containing the gradient-filled rect.

### 4.5 Masks (`masks.ts`, 28 patterns)
...
### 4.7 `@svg()` data-URI backgrounds (5 patterns)
...
- `box-shadow: 0 0 Bpx color` (always this glow form in our patterns — offsets are 0) → `feDropShadow dx=0 dy=0 stdDeviation=B/2 flood-color` on the shape's group; multiple shadows → chained primitives.
...
1. `EditPattern.tsx`: add `exportSvgPattern()` beside `exportPattern()` calling `doodleRef.current?.exportSvg({ download: true })`, with success/error toasts.
2. `EditPatternHeader.tsx`: new props `onExportSvg`, `svgExportDisabled`; new `Menu.Item` "Download SVG" (icon suggestion: `FileCode` or `ImageDown` variant from lucide) directly under "Download PNG"; `disabled` + `title` tooltip when unsupported.
3. Mobile export panel in `EditPattern.tsx`: same row, same disabled treatment.
...
The decisive advantage of the DOM-walk design is that correctness is **provable per pattern**:

1. **Pixel-parity e2e** (`e2e/svg-export.spec.ts`): for each supported pattern — load the editor at a **fixed seed** (seed is encoded in the shareable URL; verify the param name in `EditPattern.tsx`'s share-link/seed-restore logic and reuse it), then in-page via `page.evaluate`:
   - grab the `<css-doodle>` element (`div[data-pattern="<slug>"] css-doodle`, `TabbiedPattern` renders the `data-pattern` attr),
   ...
   - pixel-diff. Pass = ≥ 99% of pixels within a per-channel tolerance of ~10/255 (tune once; anti-aliasing along shape edges is the expected noise; Tier C blur/shadow patterns may need a looser threshold or SSIM-style comparison).
   - On failure, attach both PNGs + the SVG as Playwright artifacts.
   - CI strategy: a representative ~20-pattern matrix (every feature category) in the default `npm run test:e2e`; the **full 165-pattern sweep** behind a tag/env (`npm run test:e2e -- --grep @svg-full`) run before release.
...
4. **Gating test**: `coil`/`spectrum`/`pinwheel` menu item disabled with tooltip; a supported pattern's item enabled and produces a download.
...
| `packages/tabbied/src/core/types.ts` | `svgExport?: boolean` on `PatternDefinition` |
| `packages/tabbied/src/core/createPattern.ts` | `exportSvg()` on controller (+ types for its options) |
| `packages/tabbied/src/core/index.ts` | re-exports |
| `packages/tabbied/src/react/TabbiedPattern.tsx` | `exportSvg` on handle (mirror `exportImage` incl. pre-mount rejection) |
| `packages/tabbied/patterns/{coil,spectrum,pinwheel}.json` | `"svgExport": false` |
| `packages/tabbied/package.json` | `test` script (`node --test`) |
| `packages/tabbied/test/*` | unit tests for pure helpers |
| `components/edit-pattern-page/EditPattern.tsx` | `exportSvgPattern()`, mobile panel row, disabled gating |
| `components/edit-pattern-page/EditPatternHeader.tsx` (+ both `.module.css`) | menu item, disabled style + tooltip |
...
- **Parity thresholds**: anti-aliasing differences between the browser's HTML rasterization and its SVG rasterization are expected; tune once on the starter set, then freeze. If a specific pattern can't reach threshold, that's a converter bug until proven otherwise.
...
- **Future patterns**: the fail-loud rule (§3.5) plus the full-catalog sweep make regressions visible; document in the pattern-authoring notes that new presets must either stick to the supported CSS subset or set `"svgExport": false`.
