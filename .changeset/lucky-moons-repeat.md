...
Add declarative mounting: `hydrateArtworks()` reads an artwork's config off
plain `data-*` attributes and mounts every match under a root, so a static
HTML page can describe its artworks in the markup and bring them all up with
one call — no component, no build step.

`artworkConfigToAttributes()` is the inverse, and `TabbiedArtwork` now uses it
on its own placeholder. A server-rendered React page therefore emits exactly
what `hydrateArtworks` reads, which is what lets a prerendered page be
...
typed by the artwork's own option metadata rather than guessed — a
